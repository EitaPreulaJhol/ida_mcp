...
## Reading Microcode with ida_mcp

```
get_microcode("0x401000")
```
...
Returns maturity level, block count and per-block bounds. Drill into a specific block
with `get_disassembly` / `get_instruction`, or walk operands with an `ida_hexrays`
visitor inside `execute_script` (`mop_r`, `mop_n`, `mop_b` types, sizes, values).
...
**Always guard with:** `if blk.mba.maturity >= 6: return 0` (in a visitor) or check `ins` context.
...
## Writing Microcode (via `execute_script` + `ida_hexrays`)

All microcode code below runs inside `execute_script`. Import `ida_hexrays` explicitly; all `m_*`/`mop_*` constants are available. Re-decompile with `force_recompile` after each change.
...
# If you made cross-block changes (visitor with MBA access):
...
## Visitor Pattern (via execute_script only)

When you need to scan the entire MBA (e.g., to find the state variable), use `execute_script`:
...
When microcode-level modification is insufficient, use `execute_script`:
