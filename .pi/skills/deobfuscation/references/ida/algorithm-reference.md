...
**Workflow for every technique:** Read microcode → Identify the specific variant → Design a targeted fix → Apply via `execute_script` → `force_recompile` → Verify → Iterate.
...
7. **Clean up.** Invalidate with `ida_hexrays.mark_cfunc_dirty(ea)` via `execute_script`, then `force_recompile`. Hex-Rays prunes unreachable dispatcher blocks automatically.
...
- **Single-block vs full-function work?** A per-block pass is enough for NOP'ing state assignments. For full rewiring (which needs cross-block state mapping), use `execute_script` with an `ida_hexrays` visitor that holds full MBA access.
...
**For algebraically constant predicates** — use Z3 via `execute_script`:
...
Byte-level problem, byte-level fix. Use `execute_script`.
